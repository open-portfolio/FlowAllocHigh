...
public class HighContext: LowContext {
    ...
    public lazy var isRollupAssets: Bool = settings.rollupAssets

    public lazy var isGroupRelatedHoldings: Bool = settings.groupRelatedHoldings

    public lazy var isReduceRebalance: Bool = settings.reduceRebalance
    ...
    private lazy var activeHoldingAssetKeySet: AssetKeySet = Set(rawHoldingsMap.map(\.key))

    private lazy var rawFixedAssetKeys: [AssetKey] = MHolding.getAssetKeys(rawFixedHoldings, securityMap: securityMap)

    private lazy var rawVariableAssetKeys: [AssetKey] = MHolding.getAssetKeys(rawVariableHoldings, securityMap: securityMap)

    public lazy var netAllocAssetKeys: [AssetKey] = netAllocs.map(\.assetKey)
    ...
    private lazy var variableAllocs: [AssetValue] = AssetValue.getAssetValues(from: variableAllocMap)

    private lazy var netAllocs: [AssetValue] = AssetValue.getAssetValues(from: netAllocMap, orderBy: baseAllocAssetKeys)

    // MARK: - Holdings  [MHolding]

    private lazy var rawVariableHoldings: [MHolding] = MHolding.getHoldings(for: variableAccountsForStrategy, accountHoldingsMap: accountHoldingsMap)

    private lazy var acceptedVariableHoldings: [MHolding] = acceptedVariableHoldingsMap.flatMap(\.value).sorted()

    // combines the distilled (accepted) asset classes with the rejected randos
    private lazy var mergedVariableHoldings: [MHolding] = mergedVariableHoldingsMap.flatMap(\.value).sorted()

    // NOTE may include orphans
    private lazy var acceptedHoldings: [MHolding] = acceptedVariableHoldings + acceptedFixedHoldings

    // combines the distilled (accepted) asset classes with the rejected randos
    private lazy var mergedHoldings: [MHolding] = mergedVariableHoldings + mergedFixedHoldings
    ...
    private lazy var acceptedFixedHoldings: [MHolding] = acceptedFixedHoldingsMap.flatMap(\.value).sorted()

    // combines the distilled (accepted) asset classes with the rejected randos
    private lazy var mergedFixedHoldings: [MHolding] = mergedFixedHoldingsMap.flatMap(\.value).sorted()

    // MARK: - Asset Holdings Map  [AssetKey: [MHolding]]

    private lazy var rawFixedHoldingsMap: AssetHoldingsMap = MHolding.getAssetHoldingsMap(rawFixedHoldings, securityMap)

    // NOTE may include orphans!! TODO
    private lazy var acceptedFixedHoldingsMap: AssetHoldingsMap = netFixedHoldingsMapPair.accepted

    private lazy var rejectedFixedHoldingsMap: AssetHoldingsMap = netFixedHoldingsMapPair.rejected

    // combines the distilled (accepted) asset classes with the rejected randos
    private lazy var mergedFixedHoldingsMap: AssetHoldingsMap = acceptedFixedHoldingsMap.merging(rejectedFixedHoldingsMap, uniquingKeysWith: { _, _ in [] })
    ...
    private lazy var rawVariableHoldingsMap: AssetHoldingsMap = MHolding.getAssetHoldingsMap(rawVariableHoldings, securityMap)

    private lazy var acceptedVariableHoldingsMap: AssetHoldingsMap = netVariableHoldingsMapPair.accepted

    private lazy var rejectedVariableHoldingsMap: AssetHoldingsMap = netVariableHoldingsMapPair.rejected

    // combines the distilled (accepted) asset classes with the rejected randos
    private lazy var mergedVariableHoldingsMap: AssetHoldingsMap = acceptedVariableHoldingsMap.merging(rejectedVariableHoldingsMap, uniquingKeysWith: { _, _ in [] })
    ...
    // may contain orphans!
    private lazy var acceptedFixedHoldingsSummary: HoldingsSummary = .getSummary(acceptedFixedHoldings, securityMap)

    public lazy var mergedFixedHoldingsSummary: HoldingsSummary = .getSummary(mergedFixedHoldings, securityMap)

    private lazy var acceptedVariableHoldingsSummary: HoldingsSummary = .getSummary(acceptedVariableHoldings, securityMap)

    // combines the distilled (accepted) asset classes with the rejected randos
    public lazy var mergedVariableHoldingsSummary: HoldingsSummary = .getSummary(mergedVariableHoldings, securityMap)

    // NOTE: may include orphans!
    private lazy var acceptedHoldingsSummary: HoldingsSummary = .getSummary(acceptedHoldings, securityMap)

    // combines the distilled (accepted) asset classes with the rejected randos
    public lazy var mergedHoldingsSummary: HoldingsSummary = .getSummary(mergedHoldings, securityMap)

    // MARK: - AssetTickerHoldingsSummaryMap [AssetKey: [SecurityKey: HoldingsSummary]]

    // NOTE may contain orphans!
    public lazy var rawFixedAssetTickerHoldingsSummaryMap: AssetTickerHoldingsSummaryMap = HoldingsSummary.getAssetTickerSummaryMap(rawFixedHoldingsMap, securityMap)

    public lazy var rawVariableAssetTickerHoldingsSummaryMap: AssetTickerHoldingsSummaryMap = HoldingsSummary.getAssetTickerSummaryMap(rawVariableHoldingsMap, securityMap)

    // NOTE may contain orphans!
    private lazy var acceptedFixedAssetTickerHoldingsSummaryMap: AssetTickerHoldingsSummaryMap = HoldingsSummary.getAssetTickerSummaryMap(acceptedFixedHoldingsMap, securityMap)

    // combines the distilled (accepted) asset classes with the rejected randos
    public lazy var mergedFixedAssetTickerHoldingsSummaryMap: AssetTickerHoldingsSummaryMap = HoldingsSummary.getAssetTickerSummaryMap(mergedFixedHoldingsMap, securityMap)

    private lazy var acceptedVariableAssetTickerHoldingsSummaryMap: AssetTickerHoldingsSummaryMap = HoldingsSummary.getAssetTickerSummaryMap(acceptedVariableHoldingsMap, securityMap)

    // combines the distilled (accepted) asset classes with the rejected randos
    public lazy var mergedVariableAssetTickerHoldingsSummaryMap: AssetTickerHoldingsSummaryMap = HoldingsSummary.getAssetTickerSummaryMap(mergedVariableHoldingsMap, securityMap)

    // MARK: - AssetHoldingsSummaryMap  [AssetKey: HoldingsSummary]

    public lazy var rawFixedSummaryMap: AssetHoldingsSummaryMap = HoldingsSummary.getAssetHoldingsSummaryMap(rawFixedHoldingsMap, securityMap)

    public lazy var rawVariableSummaryMap: AssetHoldingsSummaryMap = HoldingsSummary.getAssetHoldingsSummaryMap(rawVariableHoldingsMap, securityMap)

    // NOTE may contain orphans!
    public lazy var acceptedFixedSummaryMap: AssetHoldingsSummaryMap = HoldingsSummary.getAssetHoldingsSummaryMap(acceptedFixedHoldingsMap, securityMap)

    // combines the distilled (accepted) asset classes with the rejected randos
    public lazy var mergedFixedSummaryMap: AssetHoldingsSummaryMap = HoldingsSummary.getAssetHoldingsSummaryMap(mergedFixedHoldingsMap, securityMap)

    public lazy var acceptedVariableSummaryMap: AssetHoldingsSummaryMap = HoldingsSummary.getAssetHoldingsSummaryMap(acceptedVariableHoldingsMap, securityMap)

    // combines the distilled (accepted) asset classes with the rejected randos
    public lazy var mergedVariableSummaryMap: AssetHoldingsSummaryMap = HoldingsSummary.getAssetHoldingsSummaryMap(mergedVariableHoldingsMap, securityMap)

    // MARK: - AccountAssetHoldingsSummaryMap  [AccountKey: [AssetKey: HoldingsSummary]]

    public lazy var rawFixedAccountHoldingsSummaryMap: AccountAssetHoldingsSummaryMap = HoldingsSummary.getAccountAssetHoldingsSummaryMap(rawFixedHoldingsMap, securityMap)

    public lazy var rawVariableAccountHoldingsSummaryMap: AccountAssetHoldingsSummaryMap = HoldingsSummary.getAccountAssetHoldingsSummaryMap(rawVariableHoldingsMap, securityMap)

    public lazy var acceptedFixedAccountHoldingsSummaryMap: AccountAssetHoldingsSummaryMap = HoldingsSummary.getAccountAssetHoldingsSummaryMap(acceptedFixedHoldingsMap, securityMap)

    // combines the distilled (accepted) asset classes with the rejected randos
    public lazy var mergedFixedAccountHoldingsSummaryMap: AccountAssetHoldingsSummaryMap = HoldingsSummary.getAccountAssetHoldingsSummaryMap(mergedFixedHoldingsMap, securityMap)

    public lazy var acceptedVariableAccountHoldingsSummaryMap: AccountAssetHoldingsSummaryMap = HoldingsSummary.getAccountAssetHoldingsSummaryMap(acceptedVariableHoldingsMap, securityMap)

    private lazy var rejectedVariableAccountHoldingsSummaryMap: AccountAssetHoldingsSummaryMap = HoldingsSummary.getAccountAssetHoldingsSummaryMap(rejectedVariableHoldingsMap, securityMap)

    // combines the distilled (accepted) asset classes with the rejected randos
    public lazy var mergedVariableAccountHoldingsSummaryMap: AccountAssetHoldingsSummaryMap = HoldingsSummary.getAccountAssetHoldingsSummaryMap(mergedVariableHoldingsMap, securityMap)
    ...
    private lazy var rollupMap: RollupMap? = netAllocMapPair.rollupMap
    ...
    public lazy var variableAllocMap: AssetValueMap = AssetValue.getNormalizedAssetValueMap(from: netVariableAssetAmountMap)

    public lazy var netAllocMap: AssetValueMap = netAllocMapPair.netAllocMap
    ...
    private lazy var fixedMapPair: FixedAllocationMapPair = allocateFixed(accounts: fixedAccountsForStrategy,
    ...
    private lazy var netVariableValueMap: AssetValueMap = MHolding.getPresentValueMap(holdingsMap: acceptedVariableHoldingsMap, securityMap: securityMap)
    ...
    public lazy var netFixedTotal: Double = AssetValue.sumOf(netFixedAssetAmountMap)
    ...
    // NOTE may include orphans!
    private lazy var netTotal: Double = getNetCombinedTotal(fixedValueMap: netFixedValueMap,
    ...
    // NOTE may include orphans!
    private lazy var netAssetTotal: Double = AssetValue.sumOf(netAssetAmountMap)

    // MARK: - AccountHoldingsMap  [AccountKey: [MHolding]]

    private lazy var rawVariableAccountHoldingsMap: AccountHoldingsMap = Dictionary(grouping: rawVariableHoldings, by: { $0.accountKey })

    // NOTE: may include orphans!
    private lazy var netAccountHoldingsMap: AccountHoldingsMap = Dictionary(grouping: acceptedHoldings, by: { $0.accountKey })

    private lazy var netVariableAccountHoldingsMap: AccountHoldingsMap = Dictionary(grouping: acceptedVariableHoldings, by: { $0.accountKey })

    // MARK: - Transaction Maps

    public lazy var assetBuyTxnsMap: AssetTxnsMap = MTransaction.getAssetTxnsMap(recentBuyTxns, securityMap)

    public lazy var assetSellTxnsMap: AssetTxnsMap = MTransaction.getAssetTxnsMap(recentSellTxns, securityMap)

    public lazy var recentPurchasesMap: RecentPurchasesMap = MTransaction.getRecentPurchasesMap(recentBuyTxns: recentBuyTxns)

    // NOTE: not considering whether account is taxable or not, as it may not be known
    public lazy var assetRecentNetGainsMap: AssetTickerAmountMap = MTransaction.getAssetNetRealizedGainMap(assetSellTxnsMap: assetSellTxnsMap,
                                                                                                           accountMap: strategiedAccountMap)

    // NOTE: not considering whether account is taxable or not, as it may not be known
    public lazy var assetRecentPurchaseMap: AssetTickerAmountMap = MTransaction.getAssetRecentPurchaseMap(assetBuyTxnsMap: assetBuyTxnsMap)
    ...
    public lazy var missingRealizedGainsTxns: [MTransaction] = recentSellTxns.filter {
        $0.needsRealizedGain(thirtyDaysBack, securityMap, strategiedAccountMap)
    }

    public lazy var recentSellTxns: [MTransaction] = recentTxns.filter { $0.isSell }

    public lazy var recentBuyTxns: [MTransaction] = recentTxns.filter { $0.isBuy }
